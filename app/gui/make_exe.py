...
includes = [ 'matplotlib.backends.backend_tkagg', 'clr', 'subprocess' ]
...
tclfiles = [
	os.path.join( os.environ[ 'PYTHONHOME_EPOCS' ], 'tcl', 'tcl8.5', 'init.tcl' ),
]
...
print "running py2exe for gusbamp..."
open( 'EpocsCommandLineArguments.py', 'wt' ).write( 'args = [ "--amp=gusbamp" ]\n' )
logfile = 'make_exe.log'; sys.stderr = sys.stdout = open( logfile, 'wt')
setup(
	options=options,
...
sys.stderr, sys.stdout = oldstderr, oldstdout
dependencies += [ x.strip().split( ' ', 2 )[ -1 ] for x in open( logfile, 'rt' ).readlines() if x.startswith( ' ' ) and ' - ' in x ]
print "running py2exe for offline..."
