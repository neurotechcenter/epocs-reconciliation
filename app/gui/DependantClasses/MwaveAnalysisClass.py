...
class MWaveAnalysisWindow(tkinter.Toplevel, TkMPL):
    ...
    def initUI(self):
        ...
        self.parent.panel = self.panel = Bunch(
            ...
            LastM=InfoItem('Current M Amplitude:', 0, fmt='%.1f', units='V', color='#000000').tkc(self.frame, column=3),
        )
        ...
        self.axes_seq.set_xlim([0, XaxisLimit + 0.5])
        ...
        self.axiscontrollers_emg2.append(
            AxisController(ax2, 'y', units='V', start=(0, +v[0]), narrowest=(0, +0.0001), widest=(0, +5.000)))
        ...
        if (self.CheckVar.get()) and (len(self.parent.MwaveLoadedData)):
            ...
            if ((var1 != None) and (var2 != None)):
                ...
                if (var2 <= 100) and (var2 > 0):
                    ...
                    #update left/right entries
                    if self.parent.operator.params._Limb == 'left': self.parent.operator.params._Left_MwaveOverlay = [var1, var2]
                    else: self.parent.operator.params._Right_MwaveOverlay = [var1, var2]
    ...
    def UpdateOverlay(self):
        ...
        comparisonInterval = self.parent.operator.params._ComparisonStartMsec[self.channel] / 1000.0, \
                             self.parent.operator.params._ComparisonEndMsec[self.channel] / 1000.0
        self.overlay.responseSelector.set(responseInterval)
        self.overlay.comparisonSelector.set(comparisonInterval)
    ...
    def PersistTimings(self):
        ...
        if self.switch.get() and not self.parent.operator.started:
            ...
            if self.overlay.comparisonSelector:
                ...
                if Limb == 'left':
                    self.parent.operator.Set(_Left_ComparisonStartMsec=[start, start],
                                             _Left_ComparisonEndMsec=[end, end])
                else:
                    self.parent.operator.Set(_Right_ComparisonStartMsec=[start, start],
                                             _Right_ComparisonEndMsec=[end, end])
            if self.overlay.responseSelector:
                start, end = [sec * 1000.0 for sec in self.overlay.responseSelector.get()]
                ...
                if Limb == 'left':
                    self.parent.operator.Set(_Left_ResponseStartMsec=[start, start], _Left_ResponseEndMsec=[end, end])
                else:
                    self.parent.operator.Set(_Right_ResponseStartMsec=[start, start], _Right_ResponseEndMsec=[end, end])
