...
class CurrentControl(object):
    ...
    def __init__(self,parent,mode,EndPoint='Mmax',RCindex=None):
        ...
        if self.mode in ['st']:
            ...
            self.CurrentLimit = self.parent.operator.params._CurrentLimit*1000   # uA
            ...
            self.Method = self.parent.operator.params._ControlMethod
    ...
    def RCProcess(self):
        ...
        def NewCurrent():
            if self.CurrentAmplitude <= self.CurrentLimit:
                ...
                self.CurrentAmplitude = self.CurrentLimit
            #self.parent.stimGUI.CurrentAmplitudeState.append(self.CurrentAmplitude)
...
class StimControl(object):
    ...
    def Update(self,NewValue):
        ...
        self.x = numpy.append(self.x,[100*(NewValue-self.Setpoint)/self.Setpoint])
        l = numpy.size(self.x)

        if l > 1:
            self.delta = numpy.append(self.delta,  self.x[l-1]-self.x[l-2])
        #else: self.delta = numpy.append(self.delta, self.x[l - 1] - self.Setpoint)

        ind1 = range(max(0,l-self.N+1),l)
        ind2 = range(max(0, l - self.N), l)
        output = (numpy.mean(self.delta[ind1]) + numpy.mean(self.x[ind2])) / 2
