...
class CheckGroup(ConnectedWidget):
    """
    A BUTTON group that can be selected like a radio button group but can be fed back to a settings parameter for storage
    """

    def __init__(self,Title='',Frame=None,Width=None,parent=None,initialValue=0, command=None, values=('EMG1','EMG2','EMG3','EMG4')):

        ConnectedWidget.__init__(self, parent)
        fontlarge = ('Helvetica', 10)

        if Frame == None: Frame = tkinter.Frame(self,parent=parent)
        subFrame = tkinter.LabelFrame(Frame,text=Title,padx=5,pady=5,font=fontlarge)
        ...
        for i in range(len(self.values)):
            w = tkinter.Radiobutton(subFrame,text=values[i],value=i+1,font=font,tristatevalue=0,selectcolor='white',command=lambda x=i: self.CheckButton(val=x),variable=self.v)
            w.grid(row=1, column=i, sticky='e', pady=1,padx=1)
            if Width != None: w.config(width=Width)
            self.GroupButtons.append(w)

        subFrame.pack(side='left', padx=3)
        self.CheckButton(val=self.values.index(initialValue))

    def CheckButton(self, val):
        """
