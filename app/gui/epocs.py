    ...
    def Incoming( self, block, queue ):
        ...
        if queue == 'Signal':
            ...
            if self.stimGUI.Automate.get() and AUTOMATION:
                ...
                if (code in 'ct'):
                    ProcessCompleted = self.ControlObject.CTTTProcess()
        ...
        if changed.TrialsCompleted: # the TrapFilter.cpp filter inside the ReflexConditioningSignalProcessing.exe module increments the TrialsCompleted state variable to indicate that a new trial has been trapped
            ...
            #if DS5 == 1:
            print self.states[code].CurrentAmplitude
...
class SettingsWindow( Dialog, TkMPL ):
    ...
    def body( self, frame ):
        ...
        # Amplifier Gain
        #self.widgets.entry_AmplifierGain = LabelledEntry(subsection, 'Amplifier Gain: ').connect(params,
        #                                                                                            '_AmplifierGain').enable(
        #    True).grid(row=1, column=1, sticky='nw', padx=2, pady=2)  # .pack(side='left', padx=3)
        #subsection.pack(fill='x', padx=10, pady=10)
        #section.pack(side='top', pady=10, padx=10, fill='both')
    ...
    def validate( self ):
        ...
        if value.SamplingRate not in [800,1200,1600,3200,6400,12800]:
