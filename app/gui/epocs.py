...
import inspect
import os,re,struct,sys,mmap
from DependantClasses import CSVfileClass
...
class Operator( object ):
    ...
    def __init__( self ):
        ...
        self.params = Bunch(   # keys without underscores are direct representations of BCI2000 parameters
            ...
            _EMGChannelNames =       [ 'EMG1', 'EMG3', ],  # "parameter" names beginning with '_' are not real BCI2000 parameters
            ...
            _LeftLimb=['EMG2', 'EMG4'],
            _RightLimb=['EMG1', 'EMG3'],
            _Limb='right',
            _Left_BackgroundMin= [5, 0],
            _Left_BackgroundMax=   [18, 15],
            _Right_BackgroundMin= [5, 0],
            _Right_BackgroundMax= [18, 15],

            _Left_ResponseStartMsec=[14, 14, ],
            _Left_ResponseEndMsec=[30, 30, ],
            _Left_ComparisonStartMsec=[4, 4, ],
            _Left_ComparisonEndMsec=[13, 13, ],
            _Left_MwaveOverlay=[30, 20],

            _Right_ResponseStartMsec=[14, 14, ],
            _Right_ResponseEndMsec=[30, 30, ],
            _Right_ComparisonStartMsec=[4, 4, ],
            _Right_ComparisonEndMsec=[13, 13, ],
            _Right_MwaveOverlay=[30, 20],
    ...
    def DataRoot( self ):
        ...
        TODO: This does not work if data is not in this path...
        """
    ...
    def SetConfig( self, work_around_bci2000_bug=False, IncrementRun=True ):
        """
        ...
        """
        if IncrementRun: self.params.SubjectRun = '%02d' % self.NextRunNumber()
...
class GUI( tksuperclass, TkMPL ):
    ...
    def __init__( self, operator=None ):
        ...
        try:
            if ('DependantClasses.StimulusControl' in sys.modules) and AUTOMATION: self.Automate = True
            else: self.Automate = False
            self.stimGUI = CurrentControl.CurrentControlWindow(parent=self,automate=self.Automate)
        except:
            pass
    ...
    def SetAutomatedParameters(self, mode=None):
        ...
        if hasattr(self, 'stimGUI'):
            self.stimGUI.CurrentAmplitudeState[ mode ] = []


        if (mode not in ['rc']) and hasattr(self, 'stimGUI'):
        ...
        if mode in ['st']:
            ...
            if hasattr(self, 'stimGUI'):
                self.stimGUI.SetNewCurrent(value=float(self.stimGUI.CurrentAmplitude))  # This will in the initial amplitude or update to what the user sets
        ...
        if mode in ['rc']:
            ...
            try:
                if hasattr(self, 'stimGUI'):
                    self.stimGUI.SetNewCurrent(value=float(self.stimGUI.CurrentAmplitude))

                    if (self.stimGUI.Automate.get() == 1) and (self.Automate):
                    ...
                    if self.Automate:
                        ...
                        #self.RCanalysis = StimControl.RCAnalysis(parent=self)
    ...
    def Start( self, mode ):
        ...
        if hasattr(self,'DS5') and (self.DigitimerEnabled == 'on') and (self.DigitimerSelection == 'DS5'):
            ...
            if self.DS5.DS5init:
                ...
                return
        mode_add = ''
        if hasattr(self,'stimGUI') and self.Automate:
            if self.stimGUI.Automate.get() == 1:
                mode_add = 'auto'

        self.run = 'R%02d' % self.operator.NextRunNumber() # must query this *before* starting the run
        self.operator.Start( mode_add + mode.upper())
        self.EnableTab( mode )
    ...
    def Stop( self ,mode):
        ...
        if hasattr(self,'stimGUI') and self.Automate: self.stimGUI.AutomateCheck.config(state='normal')
    ...
    def SetupEMGChannels(self):

        channel = self.operator.params._EMGChannelNames
        fbchannel = channel.index(self.operator.params._FeedbackChannel)

        Limb = self.operator.params._Limb
        if Limb == 'left':
            EMG1 = self.operator.params._LeftLimb[0]
            EMG2 = self.operator.params._LeftLimb[1]

            self.operator.params._BackgroundMin = self.operator.params._Left_BackgroundMin
            self.operator.params._BackgroundMax = self.operator.params._Left_BackgroundMax

            start, end = [self.operator.params._Left_ResponseStartMsec[0], self.operator.params._Left_ResponseEndMsec[0]]
            self.operator.Set(_ResponseStartMsec=[start, start], _ResponseEndMsec=[end, end])

            start, end = [self.operator.params._Left_ComparisonStartMsec[0],
                          self.operator.params._Left_ComparisonEndMsec[0]]
            self.operator.Set(_ComparisonStartMsec=[start, start], _ComparisonEndMsec=[end, end])
            self.operator.SendParameter('D188Channel',2)
            self.operator.params._MwaveOverlay =self.operator.params._Left_MwaveOverlay
            if hasattr(self,'mwaveGUI'):
                self.mwaveGUI.widgets.entry_meanVar.set(str(int(self.operator.params._Left_MwaveOverlay[0])))
                self.mwaveGUI.widgets.entry_percVar.set(str(int(self.operator.params._Left_MwaveOverlay[1])))
                self.mwaveGUI.RegionUpdate()

        else:
            # assuming no 3rd choice
            EMG1 = self.operator.params._RightLimb[0]
            EMG2 = self.operator.params._RightLimb[1]

            self.operator.params._BackgroundMin = self.operator.params._Right_BackgroundMin
            self.operator.params._BackgroundMax = self.operator.params._Right_BackgroundMax

            start, end = [self.operator.params._Right_ResponseStartMsec[0], self.operator.params._Right_ResponseEndMsec[0]]
            self.operator.Set(_ResponseStartMsec=[start, start], _ResponseEndMsec=[end, end])

            start, end = [self.operator.params._Right_ComparisonStartMsec[0],
                          self.operator.params._Right_ComparisonEndMsec[0]]
            self.operator.Set(_ComparisonStartMsec=[start, start], _ComparisonEndMsec=[end, end])
            self.operator.SendParameter('D188Channel',1)
            self.operator.params._MwaveOverlay = self.operator.params._Right_MwaveOverlay
            if hasattr(self, 'mwaveGUI'):
                self.mwaveGUI.widgets.entry_meanVar.set(str(int(self.operator.params._Right_MwaveOverlay[0])))
                self.mwaveGUI.widgets.entry_percVar.set(str(int(self.operator.params._Right_MwaveOverlay[1])))
                self.mwaveGUI.RegionUpdate()


        self.operator.params._EMGChannelNames = [EMG1,EMG2]

        #self.widgets.switch_fbchannel.value()
        if fbchannel == 0:
            self.operator.params._ResponseChannel = EMG1
            self.operator.params._FeedbackChannel = EMG1
        else:
            self.operator.params._ResponseChannel = EMG2
            self.operator.params._FeedbackChannel = EMG2

        if hasattr(self, 'mwaveGUI'): self.mwaveGUI.UpdateOverlay()
        #self.operator.
        self.operator.bci2000('set parameter Trigger list ChannelsToTrap= 2' + EMG1 + ' ' + EMG2)
        self.operator.SetConfig(IncrementRun=False)
        self.SetBarLimits('vc', 'rc', 'ct', 'tt')
        self.SetTargets('vc', 'rc', 'ct', 'tt')
        self.DrawFigures()
    ...
    def Incoming( self, block, queue ):
        ...
        if queue == 'Signal':
            ...
            if self.stimGUI.Automate.get() and self.Automate:
                if (code in 'rc'):
...
class AnalysisWindow( Dialog, TkMPL ):
    ...
    def __init__( self, parent, mode, geometry=None, modal=True, online=True ):
        ...
        if self.mode == 'offline':
            if self.parent.Channels2Trap[0][0] in self.parent.operator.params._LeftLimb:
                self.Limb = 'left'
                start, end = [self.parent.operator.params._Left_ResponseStartMsec[0],
                              self.parent.operator.params._Left_ResponseEndMsec[0]]
                self.parent.operator.Set(_ResponseStartMsec=[start, start], _ResponseEndMsec=[end, end])

                start, end = [self.parent.operator.params._Left_ComparisonStartMsec[0],
                              self.parent.operator.params._Left_ComparisonEndMsec[0]]
                self.parent.operator.Set(_ComparisonStartMsec=[start, start], _ComparisonEndMsec=[end, end])
            else:
                self.Limb = 'right'
                start, end = [self.parent.operator.params._Right_ResponseStartMsec[0],
                              self.parent.operator.params._Right_ResponseEndMsec[0]]
                self.parent.operator.Set(_ResponseStartMsec=[start, start], _ResponseEndMsec=[end, end])

                start, end = [self.parent.operator.params._Right_ComparisonStartMsec[0],
                              self.parent.operator.params._Right_ComparisonEndMsec[0]]
                self.parent.operator.Set(_ComparisonStartMsec=[start, start], _ComparisonEndMsec=[end, end])
        else:
            self.Limb = self.parent.operator.params._Limb
        ...
        if online==True:
            ...
            if mode in 'vc': EMG1channel = self.parent.operator.params._FeedbackChannel
            else: EMG1channel = self.parent.operator.params._ResponseChannel
            channels = self.parent.operator.params._EMGChannelNames
            self.channel = channels.index(EMG1channel)  # first EMG channel
            ...
            EMG1channel = self.parent.Channels2Trap[0][0]
            channels = list(self.parent.ChannelNames[0])
            channels.remove('TRIG')
            self.channel = channels.index(EMG1channel)  # first EMG channel
            if parent.Currents != []:
                c = parent.Currents
                DS5 = 1
    ...
    def PersistTimings( self ):
        ...
        if self.overlay.comparisonSelector:
            ...
            if self.Limb == 'left': self.parent.operator.Set(_Left_ComparisonStartMsec=[ start, start ],_Left_ComparisonEndMsec=[ end, end ])
            else: self.parent.operator.Set(_Right_ComparisonStartMsec=[ start, start ],_Right_ComparisonEndMsec=[ end, end ])
        if self.overlay.responseSelector:
            start, end = [ sec * 1000.0 for sec in self.overlay.responseSelector.get() ]
            self.parent.operator.Set( _ResponseStartMsec=[ start, start ], _ResponseEndMsec=[ end, end ] )
            self.parent.Log( 'Updated target response interval: %g to %g msec' % ( start, end ) )

            if self.Limb == 'left': self.parent.operator.Set(_Left_ResponseStartMsec=[ start, start ],_Left_ResponseEndMsec=[ end, end ])
            else: self.parent.operator.Set(_Right_ResponseStartMsec=[ start, start ],_Right_ResponseEndMsec=[ end, end ])

        if hasattr(self.parent,'mwaveGUI'): self.parent.mwaveGUI.UpdateOverlay()
        self.UpdateResults()
    ...
    def body( self, frame ):
        ...
        if self.mode in [ 'vc' ]:
            ...
            if self.mode != 'offline':
                ...
                if self.Limb == 'left':
                    responseInterval = self.parent.operator.params._Left_ResponseStartMsec[self.channel] / 1000.0, \
                                       self.parent.operator.params._Left_ResponseEndMsec[self.channel] / 1000.0
                    comparisonInterval = self.parent.operator.params._Left_ComparisonStartMsec[self.channel] / 1000.0, \
                                         self.parent.operator.params._Left_ComparisonEndMsec[self.channel] / 1000.0
                else:
                    responseInterval = self.parent.operator.params._Right_ResponseStartMsec[self.channel] / 1000.0, \
                                       self.parent.operator.params._Right_ResponseEndMsec[self.channel] / 1000.0
                    comparisonInterval = self.parent.operator.params._Right_ComparisonStartMsec[self.channel] / 1000.0, \
                                         self.parent.operator.params._Right_ComparisonEndMsec[self.channel] / 1000.0
            ...
            if self.mode in [ 'rc', 'ct', 'tt', 'offline', 'mixed' ]:
                ...
                w = self.widgets.sequence_button_export = tkinter.Button(header, text='Export',command=Curry(self.Export, type='sequence')); w.pack(side='right')
    ...
    def Export(self,type):
        ...
        if type == 'mvc':
            ...
            # EMG channel
            if self.mode == 'offline':
                EMGChannel = self.parent.Channels2Trap[0][0]
            ...
            try:
                if self.Limb == 'left':
                    Hwindow = [params._Left_ResponseStartMsec[0] / 1000.0, params._Left_ResponseEndMsec[0] / 1000.0]
                    Mwindow = [params._Left_ComparisonStartMsec[0] / 1000.0, params._Left_ComparisonEndMsec[0] / 1000.0]
                    Mtarget = params._Left_MwaveOverlay[0]
                    MtargetPerc = params._Left_MwaveOverlay[1]
                else:
                    Hwindow = [params._Right_ResponseStartMsec[0] / 1000.0, params._Right_ResponseEndMsec[0] / 1000.0]
                    Mwindow = [params._Right_ComparisonStartMsec[0] / 1000.0, params._Right_ComparisonEndMsec[0] / 1000.0]
                    Mtarget = params._Right_MwaveOverlay[0]
                    MtargetPerc = params._Right_MwaveOverlay[1]
            except:
                #Define H, M, BG Windows
                Hwindow = [params._ResponseStartMsec[0]/1000.0, params._ResponseEndMsec[0]/1000.0]
                Mwindow = [params._ComparisonStartMsec[0]/1000.0, params._ComparisonEndMsec[0]/1000.0]
                # M target value - left/right check
            ...
            CSVfileClass.CSVfileClass(FullFilename+'_pooled',HeaderLabels_pooled,HeaderValues_pooled,DataLabels,Data_pooled)
            CSVfileClass.CSVfileClass(FullFilename, HeaderLabels, HeaderValues, DataLabels, Data)
...
class SettingsWindow( Dialog, TkMPL ):
    ...
    def body( self, frame ):
        ...
        tkinter.Label(subsection, text='Background EMG', justify='right', state=state, bg=bg).grid(row=1, column=1,
                                                                                                   sticky='nsw', padx=2,
                                                                                                   pady=2)
        tkinter.Label(subsection, text=params._EMGChannelNames[0], justify='center', state=state, bg=bg).grid(row=1, column=2,
                                                                                           sticky='nsew', padx=10,
                                                                                           pady=2)
        tkinter.Label(subsection, text=params._EMGChannelNames[1], justify='center', state=state, bg=bg).grid(row=1, column=3,
                                                                                           sticky='nsew', padx=10,
                                                                                           pady=2)
        ...
        if DIGITIMER:
            ...
            self.widgets.switch_DigitimerEnable = Switch(subsection, title='Digitimer Enable:', offLabel='ON', onLabel='OFF',
                                                   values=['on', 'off'], initialValue='on',command=self.DigitimerSwitch).connect(params,'_DigitimerEnable').enable(state).pack(side='left', padx=10, pady=10)
        ...
        section = tkinter.LabelFrame(Stimframe, text='Treadmill Multi-muscle Settings', bg=bg)
        subsection = tkinter.Frame(section, bg=bg)
        # self, Title = '', Frame = None, Width = None, parent = None, command = None, values = ('EMG1', 'EMG2', 'EMG3', 'EMG4')
        channels = self.parent.operator.remote.GetListParameter('ChannelNames')
        channels.remove('TRIG')
        self.widgets.choice_LeftLimbEMG1 = CheckGroup(Frame=subsection, Title='Left Limb (Agonist): ',
                                                      initialValue=params._LeftLimb[0], parent=self,
                                                      values=channels).connect(params, '_LeftLimb', 0).enable(
            True).pack(side='left', padx=3)
        self.widgets.choice_LeftLimbEMG2 = CheckGroup(Frame=subsection, Title='Left Limb (Antagonist): ',
                                                      initialValue=params._LeftLimb[1], parent=self,
                                                      values=channels).connect(params, '_LeftLimb', 1).enable(
            True).pack(side='left', padx=3)
        self.widgets.choice_RightLimbEMG1 = CheckGroup(Frame=subsection, Title='Right Limb (Agonist): ',
                                                       initialValue=params._RightLimb[0], parent=self,
                                                       values=channels).connect(params, '_RightLimb', 0).enable(
            True).pack(side='bottom', padx=3)
        self.widgets.choice_RightimbEMG2 = CheckGroup(Frame=subsection, Title='Right Limb (Antagonist): ',
                                                      initialValue=params._RightLimb[1], parent=self,
                                                      values=channels).connect(params, '_RightLimb', 1).enable(
            True).pack(side='bottom', padx=3)

        # self.widgets.entry_RightLimb= LabelledEntry(subsection, 'Right Limb: ').connect(params,'_RightLimb').enable(True).pack(side='left', padx=3)
        subsection.pack(fill='x', padx=10, pady=10)
        section.pack(side='top', pady=10, padx=10, fill='both')
        ...
        self.widgets.label_message = [w1, w2, w3]
    ...
    def validate( self ):
        ...
        for key, widget in self.widgets.items():
            ...
            if key in 'isi isit_st ctCount ttCount CurrentLimit Increment Increment IncrementStart': ilabel=1
            elif key in 'HighPassCorner LowPassCorner NotchCenter SamplingRate': ilabel=3
            if x == '':
        ...
        if hasattr(value,'IncrementStart'):
            ...
            if ((value.IncrementIncrement < 0.125) or (value.IncrementIncrement > 2.5)): return self.error('Step Increments must be between 0.25 and 2.5mA', entry.IncrementIncrement,ilabel=1)
            if ((value.IncrementStart % value.IncrementIncrement) != 0): return self.error( 'Current Step start value must be an integer multiple of the step increments',value.IncrementStart, entry.IncrementIncrement,ilabel=1)
    ...
    def apply( self ):
        ...
        for k, v in sorted( changed.items() ):
            if v:
                ...
                if (k == '_BackgroundMin') or (k == '_BackgroundMax'):
                    self.SetEMGBackground(k)
                if (k== '_ResponseStartMsec') or (k== '_ResponseEndMsec'):
                    Limb = self.parent.operator.params._Limb
                    start, end = [self.parent.operator.params._ResponseStartMsec[1],
                                  self.parent.operator.params._ResponseEndMsec[1]]
                    if Limb == 'left': self.parent.operator.Set(_Left_ResponseStartMsec=[start, start], _Left_ResponseEndMsec=[end, end])
                    else: self.parent.operator.Set(_Right_ResponseStartMsec=[start, start], _Right_ResponseEndMsec=[end, end])
        ...
        #Add in setupEMG the setting of background level to left/right
        self.parent.SetupEMGChannels()
        self.parent.DrawFigures()

    def SetEMGBackground(self,k):

        if self.parent.operator.params._Limb == 'left':

            if k == '_BackgroundMin':
                self.parent.operator.params._Left_BackgroundMin = self.parent.operator.params._BackgroundMin
            else: self.parent.operator.params._Left_BackgroundMax = self.parent.operator.params._BackgroundMax
        else:
            if k == '_BackgroundMin':
                self.parent.operator.params._Right_BackgroundMin = self.parent.operator.params._BackgroundMin
            else: self.parent.operator.params._Right_BackgroundMax = self.parent.operator.params._BackgroundMax
...
class OfflineAnalysis( object ):
    ...
    def Go( self ):
        ...
        a.title( 'EPOCS Offline Analysis: ' + self.GetDescription() + ' Channel: ' + self.Channels2Trap[0][0])
    ...
    def ReadDatFile( self, filename, **kwargs ):
        ...
        p = s.ImportantParameters = Bunch(
            ...
            ChannelsToTrap = tuple(s.Parms.ChannelsToTrap.Value),
            ChannelNames = tuple(s.Parms.ChannelNames.Value),
        )
    ...
    def OpenFiles( self, filenames=None, **kwargs ):
        ...
        errs = []
        for field in 'LookBack LookForward SamplingRate SampleBlockSize ChannelsToTrap ChannelNames'.split():
            vals = unique[ field ]
        ...
        #TODO: Offline loads the actually recorded ResponseStart and End, overwriting any changes when you reload a subject. This should be a choice.

        d = self.operator.ReadSubjectSettings(suffix='')
        ...
        self.ChannelNames = unique.ChannelNames
        self.Channels2Trap = unique.ChannelsToTrap
