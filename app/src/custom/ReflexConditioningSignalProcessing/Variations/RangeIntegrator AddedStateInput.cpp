...
RangeIntegrator::Publish()
{
 BEGIN_PARAMETER_DEFINITIONS
   "Responses:Response%20Magnitudes matrix ResponseDefinition= "
               " 1 { Input%20Channel Start   End   Subtract%20Mean? Norm   Weight  Response%20Name Response%20State } "
               "         1          28ms    35ms        no          1      1.0          H		ResponseFeedbackValue "  			
...
) const
{
  ...
  if( definitionMatrix->NumColumns() != 8 )
  {
    bcierr << paramName << " parameter must have 8 columns" << endl;
    return;
  }
  ...
  for( int row = 0; row < definitionMatrix->NumRows(); row++ )
  {
	...
	//New addition wherein we can assign a ResponseDefinition to a state
	string responseStateString = StringUtils::Strip( definitionMatrix( row, ++col ) );
	if( responseStateString.size() == 0 )
      bcierr << "response states cannot be blank (" << paramName << " parameter, row " << row + 1 << ", column " << col + 1 << ")" << endl;
	if (States->Exists(responseStateString))
	  bcierr << "response state already in use (" << paramName << " parameter, row " << row + 1 << ", column " << col + 1 << ")" << endl;
	else States->Add(responseStateString + "32 0 0 0");
  ...
  //bool C = States->Exists("ReferenceFeedbackValue");
